        ...
        <Card>
            <div className="flex items-center justify-between w-full">
                <div className="flex flex-row gap-5 item-center">
                    ...
                    {startContent && <IconBox className="text-primary">{startContent}</IconBox>}

                    {/* 텍스트 박스 */}

                    <div>
                        <span className="text-slate-400 text-sm">{name}</span>
                        <strong className="block text-slate-900 text-2xl">{value}</strong>
                ...
                {endContent && <div className="w-14 h-14 rounded-full">{endContent}</div>}
            </div>
        </Card>
    );
};
